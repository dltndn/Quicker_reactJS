...
class Tmap {
    ...
    async getDistance(startPosition: coordination, arrivePosition: coordination) {
      ...
      try {
        const response = await fetch(`https://apis.openapi.sk.com/tmap/routes/distance?version=1&format=json&callback=result&startX=${startPosition.X}&startY=${startPosition.Y}&endX=${arrivePosition.X}&endY=${arrivePosition.Y}&reqCoordType=WGS84GEO`, {
          method: "GET",
          headers: headers
        });
